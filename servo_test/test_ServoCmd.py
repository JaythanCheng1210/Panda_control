...
def move(linear, angular):
    ...
    forward(servo_angle=[20, 0, 0, -20], sleepTime=0.15,
            servo_rate=[linear_x-angular_z, linear_x+angular_z])

    forward(servo_angle=[-38, 0, 0, 38], sleepTime=0.15,
            servo_rate=[linear_x-angular_z, linear_x+angular_z])

    forward(servo_angle=[-40, -40, 40, 40], sleepTime=0.15,
            servo_rate=[linear_x-angular_z, linear_x+angular_z])

    forward(servo_angle=[0, -20, 20, 0], sleepTime=0.15,
            servo_rate=[linear_x-angular_z, linear_x+angular_z])

    forward(servo_angle=[0, 38, -38, 0], sleepTime=0.15,
            servo_rate=[linear_x-angular_z, linear_x+angular_z])
